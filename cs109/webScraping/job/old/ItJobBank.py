...
class ItJobBankJob(Job):
    ...
    def parse(self, page_content):
        ...
        for i in range(0, divSize):
            ...
            if super().ex_filter(
                title, ItJobBankJob.EXECLUDE_TITLE
            ) and super().ex_filter(company, ItJobBankJob.EXECLUDE_COMPANY):
    ...
    def get_url(self, keyword, pageNum):
        return (
            "https://www.it-jobbank.dk/job/software-webudvikling/storkoebenhavn?q="
            + keyword
            + "&page="
            + str(pageNum)
        )
